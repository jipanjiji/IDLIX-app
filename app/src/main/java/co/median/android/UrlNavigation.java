...
//		Log.d(TAG, "shouldOverrideUrl: " + url);
        ...
        if (!isInternalUri(uri)) {
            ...
            // Suppress ad redirects and unwanted external launches
            String host = uri.getHost();
            if (host != null) {
                String lowerHost = host.toLowerCase();
                if (lowerHost.contains("pop") || lowerHost.contains("ad") || lowerHost.contains("bet") ||
                    lowerHost.contains("slot") || lowerHost.contains("track") || lowerHost.contains("click") ||
                    lowerHost.contains("traffic") || lowerHost.contains("syndication") || lowerHost.contains("doubleclick")) {
                    Log.d(TAG, "Suppressed external ad redirect: " + uri);
                    return true;
                }
            }

            if ("market".equals(uri.getScheme()) || ("intent".equals(uri.getScheme()) && uri.toString().contains("market://"))) {
                Log.d(TAG, "Suppressed store redirect intent: " + uri);
                return true;
            }

            // If video is in fullscreen, completely block app browser popups
            if (mainActivity.getFullScreenLayout() != null && mainActivity.getFullScreenLayout().getVisibility() == View.VISIBLE) {
                Log.d(TAG, "Blocked external popup during fullscreen playback: " + uri);
                return true;
            }
    ...
    private void runGonativeDeviceInfo(String callback) {
        ...
        installationInfo.putAll(mainActivity.getGNApplication().mBridge.getExtraDeviceInfo(mainActivity));
...
//        Log.d(TAG, "intercept " + url);
    ...
    public void createNewWindow(WebView webView, Message resultMsg) {
        // Intercept ad popups/popunders created via window.open()
        try {
            ...
            newWebView.setWebViewClient(new WebViewClient() {
                ...
                public boolean shouldOverrideUrlLoading(WebView view, String url) {
                    ...
                    Uri uri = Uri.parse(url);
                    if (isInternalUri(uri) || url.contains("idlixku") || url.contains("idlix")) {
                        mainActivity.loadUrl(url);
                    } else {
                        Log.d(TAG, "Suppressed external ad popup window: " + url);
                    }
                    return true;
                }
            });
        } catch (Exception e) {
            Log.e(TAG, "Error handling createNewWindow", e);
        }
    }
